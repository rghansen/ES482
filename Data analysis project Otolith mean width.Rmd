...
#Summarize herring data by mean otilith for each salmon
herring_data <- herring_data %>% group_by(sal.id, sal.length, coll.doy, lat, long, coll.month) %>% summarize(oto.width = mean(oto.width))
...
*Rewrite this 
Otolith width (mm) measurements will be used as a proxy for herring size (Stevenson and Campana, 1992) in this analysis. 

We will also use salmon catch data recorded by each angler and submitted with the associated stomach sample.
...
##B) 
The response variable (otolith width) is continuous and cannot take on zero or negative values. The range of possible response values in this dataset is 0.469 mm to 2.27 mm. 

##C)
Outliers at < 0.6 mm and > 2.2 mm are reasonable measurements representing means that included individuals from the extreme age classes. There are no zero values and one missing otolith measurement making up 0.36 % of the data. We anticipate no difficulties in removing the single NA value from subsequent analyses. Herring consumed at similar times of year may have similar growth trajectories that contribute to temporal autocorrelation. Spatial autocorrelation may occur if herring consumed in similar areas have similar prey fields and grow rates

*Rewrite this
Non-independence was expected from repeated measurements of multiple herring that occurred within the same salmon stomach, which is why we are using mean otolith width for all herring from the same salmon stomach.
...
##A)
```{r, echo = TRUE}
#create data frame for pairwise scatter of continuous explanatory variables
...
A boxplot of mean salmon length per month shows that mean salmon length in summer months overlapped with mean salmon length observed in earlier months in the year, likely due to unbalanced sampling effort over the year as opposed to the concave up shape observed in the pairwise scatterplot. The remaining relationships observed in the pairwise scatterplots were consistent with the pairwise correlation coefficients. Salmon length was moderately, positively correlated (0.385) with latitude but moderately, negatively correlated longitude (-0.356). There was a strong, negative correlation (-0.874) between latitude and longitude. VIF values of latitude (4.767413) and longitude (4.69011) were greater than 3 and strongly colinear, which is expected due to the geographical constraints of the Strait of Georgia and Juan de Fuca. We will run models with either latitude or longitude to avoid the confounding effects of including both variables in a single model.
...
oto.width ~ coll.doy + lat + sal.length
oto.width ~ coll.doy + long + sal.length 

1. oto.width ~ coll.doy + lat + coll.doy:lat
2. oto.width ~ coll.doy + sal.length + coll.doy:sal.length 
3. oto.width ~ lat + sal.length + lat:sal.length 
4. oto.width ~ coll.doy + long + coll.doy:long 
5. oto.width ~ long + sal.length + long:sal.length
