...
#Two explanatory and one interaction models
...
#Three explanatory variable and one interaction models
...
#Three explanatory variable and two interaction models
...
#Residuals vs. fitted shown with smoothing curve
...
#Residuals vs. long shown with smoothing curve
ggplot(herring_data) +
  theme_minimal() + 
  geom_point(aes(long_standardized, resid_11), shape = 1) +
  geom_smooth(aes(long_standardized, resid_11), se = FALSE)

#Residuals vs. coll.doy shown with smoothing curve
ggplot(herring_data) +
  theme_minimal() + 
  geom_point(aes(coll.doy_standardized, resid_11), shape = 1) +
  geom_smooth(aes(coll.doy_standardized, resid_11), se = FALSE)

#Residuals vs. sal.length shown with smoothing curve
ggplot(herring_data) +
  theme_minimal() + 
  geom_point(aes(sal.length_standardized, resid_11), shape = 1) +
  geom_smooth(aes(sal.length_standardized, resid_11), se = FALSE)
```
...
confint_fixed <- as.data.frame(confint(model_11, parm = c("(Intercept)", "coll.doy_standardized", "long_standardized", "sal.length_standardized")))

##Compute profile confidence intervals
confint_fixed <- as.data.frame(confint_fixed)
confint_fixed$term <- rownames(confint_fixed)
confint_fixed$estimate <- fixef(model_11)

##ggplot
ggplot(confint_fixed) + 
  theme_bw() +
  ...
  xlab("") +
  ylab("") + 
  ggtitle("Coefficient Estimates for model4_standardized") +
...
herring_data$fit_11_pop <- predict(model_11, re.form = ~ 0)

#Generate model predictions at salmon id level
herring_data$fit_11_sal.id <- predict(model_11)

#Plot the model with longitude
ggplot(herring_data) +
  theme_minimal() + 
  geom_point(aes(long_standardized, oto.width), shape = 1, size = 0.2) +
  geom_line(aes(long_standardized, fit_11_pop)) + 
  geom_point(aes(long_standardized, fit_11_sal.id, group = sal.id), shape = 2)

#plot model with lat
ggplot(herring_data) +
  theme_minimal() + 
  geom_point(aes(lat_standardized, oto.width), shape = 1, size = 0.2) +
  geom_line(aes(lat_standardized, fit_11_pop)) + 
  geom_point(aes(lat_standardized, fit_11_sal.id, group = sal.id), shape = 2)

#plot model with salmon length
ggplot(herring_data) +
  theme_minimal() + 
  geom_point(aes(sal.length_standardized, oto.width), shape = 1, size = 0.2) +
  geom_line(aes(sal.length_standardized, fit_11_pop)) + 
  geom_point(aes(sal.length_standardized, fit_11_sal.id, group = sal.id), shape = 2)

#plot model with collection day
ggplot(herring_data) +
  theme_minimal() + 
  geom_point(aes(coll.doy_standardized, oto.width), shape = 1, size = 0.2) +
  geom_line(aes(coll.doy, fit_11_pop)) + 
  geom_point(aes(coll.doy, fit_11_sal.id, group = sal.id), shape = 2)

#plot model with collection day *UNSTANDARDIZED
ggplot(herring_data) +
  theme_minimal() + 
