...
oto.width ~ coll.doy + lat + doy*lat + (1 | sal.id)
...
model1 <- lmer(oto.width ~ coll.doy + sal.length + coll.doy*sal.length + (1 | sal.id), data = herring_data, REML = FALSE)
summary(model1)

model1_int <- lmer(oto.width ~ coll.doy + lat + coll.doy*lat + (1 | sal.id), data = herring_data, REML = FALSE)
summary(model1_int)
