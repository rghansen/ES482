...
ggplot(herring_data) +
  ...
  geom_smooth(aes(fitted_11, resid_11), se = FALSE) +
   xlab("Fitted values for Model 11") +
  ylab("Residuals for Model 11")
...
ggplot(herring_data) +
  ...
  xlab("Residuals for Model 6") +
  ylab("Fitted values for Model 6")
...
lat_vs_long <- ggplot(herring_data) +
  theme_minimal() + 
  geom_point(aes(long, lat, colour = sign, size = abs(oto.width_standardized), shape = sign)) + scale_colour_manual( values = c("1" = "darkorange", "-1" = "steelblue4", "0" = "black")) + 
  scale_shape_manual(values = c("1" = 1, "-1" = 1, "0" = 19)) + 
  coord_fixed() +
  ggtitle("Bubble plot of otolith width") +
  xlab("Longitude [°]") +
  ylab("Latitude [°]")
print(lat_vs_long)
```
