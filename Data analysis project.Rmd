...
#check data frame structure* do we need to change data types for any variables? i.e. coll.doy, sal.id?
str(herring_data)
...
We have found Pacific Herring are the dominant fish consumed throughout regions of the Salish Sea, year-round. For this project we will investigate factors that may be important to the size of herring consumed by Chinook Salmon. Previous work suggests a positive, linear relationship between otolith width (mm) and length (mm) of herring from this dataset. Otolith width (mm) measurements averaged from left and right otoliths will be used as a proxy for herring size (Stevenson and Campana, 1992) in this analysis. We will also use salmon catch data recorded by each angler and submitted with the associated stomach sample.

##B)
How does Chinook size, catch location and time of year in the Salish Sea affect the size of herring prey consumed?

##C)
This analysis is inferential with the goal of understanding factors that explain variation in herring size consumed by Chinook in the Salish Sea.
...
herring_collinearity <- data.frame("oto.width" = herring_data$oto.width, "sal.length" = herring_data$sal.length, "coll.doy" = herring_data$coll.doy, "lat" = herring_data$lat, "long" = herring_data$long)

plot(herring_collinearity[1:5], cex = 0.5, pch = 19, col = rgb(0, 0, 0, 0.5), 
                   labels = c("Otolith Width (mm)", "Chinook\nLength [mm]", "Collection\nDay in 2018", "Latitude", "Longitude"))
```
...
model1 <- lmer(oto.width ~ coll.doy + lat + coll.doy*lat + (1 | sal.id), data = herring_data, REML = FALSE)
model2 <- lmer(oto.width ~ coll.doy + sal.length + coll.doy*sal.length + (1 | sal.id), data = herring_data, REML = FALSE)
model3 <- lmer(oto.width ~ lat + sal.length + lat*sal.length + (1 | sal.id), data = herring_data, REML = FALSE)
model4 <- lmer(oto.width ~ coll.doy + long + coll.doy*long + (1 | sal.id), data = herring_data, REML = FALSE)
model5 <- lmer(oto.width ~ long + sal.length + long*sal.length + (1 | sal.id), data = herring_data, REML = FALSE)
...
hist(resid(model1), prob = TRUE, col = "lightgrey")
rug(resid(model1))
sd_hat <- sd(resid(model1))
curve(dnorm(x, sd = sd_hat), add = TRUE)

#make quantile-quantile plot and add reference line
qqnorm(resid(model1))
...
qqnorm(ranef(model1)$sal.id[, "(Intercept)"])
qqline(ranef(model1)$sal.id[, "(Intercept)"])
```
...
#standardize/scale the coefficients
#what this does: "standardisation by subtracting the mean and dividing by two standard deviations"
model1_scaled <- arm::standardize(model1)

#conﬁdence intervals for coeﬃcient estimate
#note: cannot compute pvalues because the number of herring per salmon is unbalanced - chapter 5, page 9
confint(model1_scaled)

#rearranging data to make coefficient plots to show standard errors:

#data frame for coefficient plot
confint_fixed <- as.data.frame(confint(model1_scaled, parm = c("(Intercept)", "z.coll.doy", "z.lat", "z.coll.doy:z.lat")))

#compute profile confidence intervals
confint_fixed <- as.data.frame(confint_fixed)
confint_fixed$term <- rownames(confint_fixed)
confint_fixed$estimate <- fixef(model1_scaled)
...
scale_y_discrete(labels = c("Intercept", "Collection Day of Year", "Latitude", "Collection Day of Year : Latitude")) +
xlab("") +
ylab("") + ggtitle("Coefficient Estimates for model1_scaled") +
