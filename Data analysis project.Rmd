...
## Section 2: The Response Variable
### A)
...
hist(herring_data$sal.length, cex.main = 1.25, cex.lab = 1, cex.axis = 1, col = grey(0.9), breaks = seq(40, 100, by = 1), main = "Histogram of salmon length", xlab = "Salmon length [cm]")
rug(herring_data$sal.length)

#Collection day of year histogram
hist(herring_data$coll.doy, cex.main = 1.25, cex.lab = 1, cex.axis = 1, col = grey(0.9), breaks = seq(0, 400, by = 7), main = "Histogram of day of year", xlab = "Collection day of year")
rug(herring_data$coll.doy)

#Latitude histogram
hist(herring_data$lat, cex.main = 1.25, cex.lab = 1, cex.axis = 1, col = grey(0.9), breaks = seq(48.0, 51.0, by = 0.05), main = "Histogram of latitude", xlab = "Latitude [°]")
rug(herring_data$lat)

#Longitude histogram
hist(herring_data$long, cex.main = 1.25, cex.lab = 1, cex.axis = 1, col = grey(0.9), breaks = seq(-125.5, -123, by = 0.05), main = "Histogram of longitude", xlab = "Longitude [°]")
...
with(herring_data, plot(sal.id, row, las = 1, type = "n", cex.lab = 1.5, cex.axis = 1, cex.main = 1.5, xlab = "Salmon ID", ylab = "Row Number", main = "Dotchart of Salmon ID"))
abline (h = seq(1, n, by = 5), col = "lightgrey")
with(herring_data, points(sal.id, row, las = 1, type = "p", pch = 19, cex = 0.5))

#Make a histogram of herring frequency per salmon
hist(herring_data$sal.id, col = grey(0.9), cex.main = 1.5, cex.lab = 1.5, cex.axis = 1, breaks = seq(17300, 18200, by = 1), main = "Histogram of Salmon ID", xlab = "Salmon ID")
```
...
### A)
```{r, echo = TRUE, fig.width = 6, fig.height = 6, echo = FALSE}
...
```{r}
#Otolith width vs. longitude
ggplot(herring_data) + theme_minimal() + geom_point(aes(long, oto.width), shape = 1) + xlab("Longitude [°]") +  ylab("Otolith width [mm]") + scale_y_continuous(breaks = seq(0, 3, 0.5))
ggsave("long.png", width = 20, height = 10, units = "cm")

#Otolith width vs. coll.doy
ggplot(herring_data) +  theme_minimal() + geom_point(aes(coll.doy, oto.width), shape = 1) + xlab("Collection of day") + ylab("Otolith width [mm]") + scale_y_continuous(breaks = seq(0, 3, 0.5))
ggsave("colldoy.png", width = 20, height = 10, units = "cm")

#Otolith width vs. sal.length
ggplot(herring_data) + theme_minimal() + geom_point(aes(sal.length, oto.width), shape = 1) + xlab("Salmon length [cm]") + ylab("Otolith width [mm]") + scale_y_continuous(breaks = seq(0, 3, 0.5))
ggsave("sallength.png", width = 20, height = 10, units = "cm")

#Otolith width vs. latitude
ggplot(herring_data) + theme_minimal() + geom_point(aes(lat, oto.width), shape = 1) + xlab("Latitude [°]") +  ylab("Otolith width [mm]") + scale_y_continuous(breaks = seq(0, 3, 0.5))
ggsave("lat.png", width = 20, height = 10, units = "cm")
```

### Plotting residuals for Model 11 and Model 6 (when predictions exclude random intercepts)
```{r}
...
resid_vs_fitted_11 <- ggplot(herring_data) + theme_minimal() + geom_point(aes(fitted_11, resid_11), shape = 1) + geom_smooth(aes(fitted_11, resid_11), se = FALSE) + xlab("Fitted values for Model 11") + ylab("Residuals for Model 11") + theme(axis.title =  element_text(size = 12, colour = "black"))

#Plot Model 6 residuals versus fitted values (shown with a smoothing curve)
resid_vs_fitted_6 <- ggplot(herring_data) + theme_minimal() + geom_point(aes(fitted_6, resid_6), shape = 1) + geom_smooth(aes(fitted_6, resid_6), se = FALSE) + ylab("Residuals for Model 6") +  xlab("Fitted values for Model 6") + theme(axis.title =  element_text(size = 12, colour = "black"))
...
#Plot histogram of Model 11 intercepts as probability density with rug marks and reference curve
...
#Plot histogram of intercepts as probability density with rug marks and reference curve
...
### Plotting Model 6 residuals vs. salmon length
```{r, fig.width = 8, fig.height = 4, echo = FALSE}
#Plot residuals vs. sal.length (shown with smoothing curve)
ggplot(herring_data) +  theme_minimal() + geom_point(aes(sal.length, resid_6), shape = 1) + geom_smooth(aes(sal.length, resid_6), se = FALSE) + ylab("Residuals for Model 6") + xlab("Salmon length [cm]")
```
### Checking for temporal and spatial autocorrelation
```{r}
...
acf(herring_data$oto.width_, las = 1, lag = 100, cex.main = 2, cex.lab = 1.5, cex.axis = 1, main = "Series for standardized otolith widths")
```
...
#Call variation within and between the random effect (salmon ID) of Model 6
VarCorr(model_6)
```

```{r, fig.width = 8, fig.height = 4, echo = FALSE}
#Calculate conﬁdence intervals for coeﬃcient estimates of Model 11
confint(model_11)

#Make data frame for plot of coefficients for Model 11
...
#Compute profile confidence intervals for coefficient estimates for Model 11
...
#Plot confidence intervals for coefficient estimates for Model 11
confint_11 <- ggplot(confint_fixed_11) + theme_bw() + geom_point(aes(y = term, x = estimate), colour = "midnightblue") + geom_vline(xintercept = 0, linetype = "dotted", colour = "red") +  geom_errorbarh(aes(y = term, xmin = `2.5 %`, xmax = `97.5 %`), colour = "midnightblue", height = 0.3) + scale_y_discrete(labels = c("Intercept", "Collection \nDay of Year", "Longitude", "Salmon length")) + xlab("Estimate") +  ylab("") +  ggtitle("Coefficient Estimates for Model 11") + scale_x_continuous(breaks = seq(-0.5, 1.5, 0.25)) + theme(axis.text = element_text(size = 14, colour = "black"), plot.title = element_text(size = 16))

confint_11
ggsave("model_11_confint.png", width = 25, height = 12.5, units = "cm")
```

```{r include=FALSE}
#Calculate conﬁdence intervals for coeﬃcient estimates of Model 6
confint(model_6)
...
#Make data frame for plot of coefficients for Model 6
confint_fixed_6 <- as.data.frame(confint(model_6, parm = c("(Intercept)", "coll.doy_standardized", "long_standardized")))

#Compute profile confidence intervals for coefficient estimates for Model 6
...
#Plot confidence intervals for coefficent estimates for Model 6
confint_6 <- ggplot(confint_fixed_6) + theme_bw() +  geom_point(aes(y = term, x = estimate), colour = "midnightblue") + geom_vline(xintercept = 0, linetype = "dotted", colour = "red") +
...
grid.arrange(confint_11, confint_6, nrow = 2) 
```
...
ggsave("model_11_long.png", width = 16, height = 8, units = "cm")
...
ggsave("model_11_coll.doy.png", width = 16, height = 8, units = "cm")
...
ggsave("model_11_sal.length.png",  width = 16, height = 8, units = "cm")
