...
acf(herring_data$oto.width_, las = 1, lag = 100, cex.main = 1.5, cex.lab = 1, cex.axis = 1, main = "Series for otolith width")
```
...
lat_vs_long <- ggplot(herring_data) + theme_minimal() + geom_point(aes(long, lat, colour = Sign, size = Magnitude, shape = Sign)) + scale_colour_manual( values = c("1" = "darkorange", "-1" = "steelblue4", "0" = "black")) + scale_shape_manual(values = c("1" = 1, "-1" = 1, "0" = 19)) + coord_fixed() +  ggtitle("Bubble plot of otolith width") + xlab("Longitude [°]") + ylab("Latitude [°]")
print(lat_vs_long)
```
